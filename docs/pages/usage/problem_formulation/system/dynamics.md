...
The {class}`opt_system` class in the {ref}`System <index_systems>` page involves networks and controllers that are only  Linear Time Invariant (LTI). 
...
The {{osyn}} implementation of periodic-orbit systems is simplified. The matrix $M \in \R^{c \times c}$ must satisfy $M = M^\top, M^h = M$, where $c$ is the coordinate dimension of the Kronecker Structure. 
